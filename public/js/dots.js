...
// Wait until socket sends raster to paint flip-dot correct size
socket.on('raster', function(size) {
  // Canvas dots taken from http://cobwwweb.com/animated-dot-background-canvas
  ...
  numCols = size[1];
    ...
    var dotWidth = ((canvasWidth - (2 * dotMargin)) / size[1]) - dotMargin;
    var dotHeight = ((canvasHeight - (2 * dotMargin)) / size[0]) - dotMargin;

    // Now, we use the limiting dimension to set the diameter.
    if( dotWidth > dotHeight ) {
      var dotDiameter = dotHeight;
      var xMargin = (canvasWidth - ((2 * dotMargin) + (size[1] * dotDiameter))) / size[1];
      var yMargin = dotMargin;
    } else {
      var dotDiameter = dotWidth;
      var xMargin = dotMargin;
      var yMargin = (canvasHeight - ((2 * dotMargin) + (size[0] * dotDiameter))) / size[0];
    }
  ...
  for(var i = 0; i < numRows; i++) { // i is the row iterator
    ...
    drawDot(i, j, size, false);
    }
  }

  canvas[0].addEventListener('mousedown', function(evt) {
    var mousePos = getMousePos(canvas[0], evt);
    pos = getDot(mousePos.x, mousePos.y, dotRadius, [dotMargin, dotDiameter, xMargin, yMargin]);
...
function updateDisplay(data) {
  ...
  for (var j = 0; j < 56; j++) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      drawDot(i,j,[numRows, numCols],bit);
    }
  }
}
...
function drawDot(i, j, size, active) {
    // Because we don't know which direction (x vs. y) is the limiting sizing
    // factor, we'll calculate both first.
    var dotWidth = ((canvasWidth - (2 * dotMargin)) / size[1]) - dotMargin;
    var dotHeight = ((canvasHeight - (2 * dotMargin)) / size[0]) - dotMargin;
...
function getDot(x, y, radius, [dotMargin, dotDiameter, xMargin, yMargin]) {
  return {
